...
static kmem_cache_t *devsw_cache;
...
{
  int buf_size = 32;
  char *buf = kmalloc(buf_size); // move batches from user space to uart.
  int i = 0;
...
consoleinit(void)
{
  ...
  devsw_cache = kmem_cache_create("devsw", sizeof(struct devsw), 0, 0);
  for (int i = 0; i < NDEV; i++)
  {
    devsw[i] = kmem_cache_alloc(devsw_cache);
    devsw[i]->read = 0;
    devsw[i]->write = 0;
  }
